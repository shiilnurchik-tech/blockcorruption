...
public abstract class BreakablePortalEntity extends Portal {
    ...
    private BreakablePortalEntity getReversePortal() {
        ...
        ServerLevel world = ((ServerLevel) level()).getServer().getLevel(getDestDim());
...
//            limitedLogger.err("Missing Reverse Portal " + this);
    ...
    public void markShouldBreak() {
        ...
        if (reversePortal != null) {
            ...
            ServerTaskList.of(((ServerLevel) level()).getServer()).addTask(MyTaskList.withRetryNumberLimit(
                30,
                () -> {
                    if (this.isRemoved()) {
                        return true;
