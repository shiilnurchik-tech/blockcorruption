...
public class GeometryPortalShape {
    ...
    public static @Nullable Mesh2D readOldMeshFromTag(ListTag tag) {
        ...
        for (int i = 0; i < triangleNum; i++) {
            mesh.addTriangle(
                tag.getDoubleOr(i * 6 + 0, 0),
                tag.getDoubleOr(i * 6 + 1, 0),
                tag.getDoubleOr(i * 6 + 2, 0),
                tag.getDoubleOr(i * 6 + 3, 0),
                tag.getDoubleOr(i * 6 + 4, 0),
                tag.getDoubleOr(i * 6 + 5, 0)
            );
        }
    ...
    public static @Nullable Mesh2D readOldMeshFromTagNonNormalized(
        ...
        for (int i = 0; i < triangleNum; i++) {
            mesh.addTriangle(
                tag.getDoubleOr(i * 6 + 0, 0) / halfWidth,
                tag.getDoubleOr(i * 6 + 1, 0) / halfHeight,
                tag.getDoubleOr(i * 6 + 2, 0) / halfWidth,
                tag.getDoubleOr(i * 6 + 3, 0) / halfHeight,
                tag.getDoubleOr(i * 6 + 4, 0) / halfWidth,
                tag.getDoubleOr(i * 6 + 5, 0) / halfHeight
            );
        }
